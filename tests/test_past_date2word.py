...
def test_hour_ar():
    ...
    assert past_date2word(time) == "منذ 5 ساعات و 5 دقائق"
...
def test_day_ar():
    ...
    assert past_date2word(time) == "منذ 5 ايام و 5 ساعات "
...
def test_week_ar():
    ...
    assert past_date2word(time) == "منذ 3 اسابيع و 4 ايام"
...
def test_month_ar():
    ...
    time = datetime.now() - timedelta(weeks=(one_month_to_week * 3) + 2)
    assert past_date2word(time) == "منذ 3 شهور واسبوعين"
    time = datetime.now() - timedelta(weeks=(one_month_to_week * 11) + 3)
    assert past_date2word(time) == "منذ 11 شهر و 3 اسابيع"
...
def test_year_ar():
    ...
    assert past_date2word(time) == " منذ سنتين وشهرين"
    time = datetime.now() - timedelta(weeks=(one_year_to_week * 5)+(one_month_to_week * 5))
    assert past_date2word(time) == "منذ 5 سنوات و 5 شهور"
    time = datetime.now() - timedelta(
        weeks=(one_year_to_week * 12) + (one_month_to_week * 5)
    )
    assert past_date2word(time) == "منذ 12 سنة و 5 شهور"